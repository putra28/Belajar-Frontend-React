...
const CekdataAktifitas = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/logaktifitas/getlogaktifitas`, { headers });
      const result = await response.json();

      if (result.status === 200) {
        setData(result.data);
        showToastMessage('Berhasil Mengambil Data Histori Aktifitas', 'Berhasil')
      } else {
        setData([]);
        showToastMessage('Gagal Mengambil Data Histori Aktifitas', 'Gagal');
      }
    } catch (error) {
